...
def create_net_argparser(*, netname, description):
    ...
    parser.add_argument(
        '--home',
        default=os.path.expanduser(f'~/.near/{netname}'),
        help=
        f'Home path for storing configs, keys and chain data (Default: ~/.near/{netname})'
    )
    ...
    parser.add_argument('--account-id', help='Specify account ID to init')
    return parser
